...
/*
    ...
    */
    ...
    private static readonly long serialVersionUID = 8987399626114087514L;

    private string alg;
    private int maxKeySize = int.MaxValue; // no restriction on maxKeySize
    private string exemptionMechanism = null;
    private AlgorithmParameterSpec algParamSpec = null;
    private bool checkParam = false; // no restriction on param

    static readonly string ALG_NAME_WILDCARD = "*";
     ...
     */
    CryptoPermission(string alg):base(null)
    {
        this.alg = alg;
    }
     ...
     */
    CryptoPermission(string alg, int maxKeySize):base(null)
    {
        this.alg = alg;
    ...
    CryptoPermission(string alg,
                     int maxKeySize,
                     AlgorithmParameterSpec algParamSpec):base(null)
    {
        this.alg = alg;
        this.maxKeySize = maxKeySize;
     ...
     */
    CryptoPermission(string alg,
                     string exemptionMechanism):base(null)
    {
        this.alg = alg;
    ...
    {
        if (!(p is CryptoPermission))
            return false;

        CryptoPermission cp = (CryptoPermission)p;

        if ((!alg.Equals(cp.alg,StringComparison.OrdinalIgnoreCase)) &&
            (!alg.Equals(ALG_NAME_WILDCARD,StringComparison.OrdinalIgnoreCase)))
        {
            return false;
        }
    ...
    public bool equals(object obj)
    {
        ...
        if (!(obj is CryptoPermission))
            return false;

        CryptoPermission that = (CryptoPermission)obj;

        if (!(alg.Equals(that.alg,StringComparison.OrdinalIgnoreCase)) ||
    ...
    public int hashCode()
    {
        int retval = alg.GetHashCode();
        retval ^= maxKeySize;
        if (exemptionMechanism != null)
        {
            retval ^= exemptionMechanism.GetHashCode();
        }
     ...
     * object.
     */
    private string getExemptionMechanism()
    {
    ...
    public string toString()
    {
        ...
        if (algParamSpec != null)
        {
            if (algParamSpec is RC2ParameterSpec) {
                buf.Append(" , effective " +
                    ((RC2ParameterSpec)algParamSpec).getEffectiveKeyBits());
            } else if (algParamSpec is RC5ParameterSpec) {
                buf.Append(" , rounds " +
    ...
    private bool impliesExemptionMechanism(String exemptionMechanism)
    {
    ...
    private bool impliesParameterSpec(bool checkParam,
                                         AlgorithmParameterSpec algParamSpec)
    {
        if ((this.checkParam) && checkParam)
        {
            ...
            if (algParamSpec is RC5ParameterSpec) {
            ...
            if (algParamSpec is PBEParameterSpec) {
 ...
 * @author Sharon Liu
 */
sealed class CryptoPermissionCollection : PermissionCollection, ISerializable
{
    private static readonly long serialVersionUID = -511215555898802763L;
...
private Vector<Permission> permissions;
     ...
     * object.
     */
    public CryptoPermissionCollection()
{
...
public void add(Permission permission)
{
    ...
    if (!(permission is CryptoPermission))
            return;
