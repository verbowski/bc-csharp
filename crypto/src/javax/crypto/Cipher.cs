...
namespace javax.crypto
{
   ...
   */
    ...
    */
    ...
    private static readonly Debug pdebug =
                        Debug.getInstance("provider", "Provider");
    private static readonly bool skipDebug =
    ...
    public static readonly int ENCRYPT_MODE = 1;
         ...
         */
        public static readonly int WRAP_MODE = 3;

        /**
         * Constant used to initialize cipher to key-unwrapping mode.
         */
        public static readonly int UNWRAP_MODE = 4;

        /**
         * Constant used to indicate the to-be-unwrapped key is a "public key".
         */
        public static readonly int PUBLIC_KEY = 1;

        /**
         * Constant used to indicate the to-be-unwrapped key is a "private key".
         */
        public static readonly int PRIVATE_KEY = 2;

        /**
         * Constant used to indicate the to-be-unwrapped key is a "secret key".
         */
        public static readonly int SECRET_KEY = 3;
        ...
        private static readonly string KEY_USAGE_EXTENSION_OID = "2.5.29.15";
    ...
    private CipherSpi firstSpi;
        ...
        private Cipher(CipherSpi firstSpi, Service firstService,
                ...
                System.Collections.Generic.List<Transform> transforms)
        {
...
//                throws NoSuchAlgorithmException
        ...
        try {
            ...
            if (count == 0 || count == 2 || parser.hasMoreTokens()) {
                throw new java.security.NoSuchAlgorithmException("Invalid transformation"
                                               + " format:" +
                                               transformation);
}
    ...
    private readonly static string ATTR_MODE = "SupportedModes";
// Provider attribute name for supported padding names
private readonly static string ATTR_PAD = "SupportedPaddings";

// constants indicating whether the provider supports
// a given mode or padding
private readonly static int S_NO = 0;       // does not support
private readonly static int S_MAYBE = 1;       // unable to determine
private readonly static int S_YES = 2;       // does support
...
private class Transform
{
        ...
        Transform(string alg, string suffix, string mode, string pad)
    {
        this.transform = alg + suffix;
                this.suffix = suffix.ToUpper(Locale.ENGLISH);
        this.mode = mode;
        this.pad = pad;
    }
...
private sealed static ConcurrentMap<string, Pattern> patternCache =
    new ConcurrentHashMap<string, Pattern>();

private static bool matches(string regexp, string str)
{
    ...
    return pattern.matcher(str.ToUpper(Locale.ENGLISH)).matches();
}
...
{
    string []
    parts = tokenizeTransformation(transformation);
...
{
    string alg = s.getAlgorithm().ToUpper(Locale.ENGLISH);
...
List<ServiceId> cipherServices = new List<ServiceId>(transforms.Count);
...
cipher.provider = s.getProvider();
        ...
        if (failure is NoSuchPaddingException) {
            ...
            if (w >= 0)
            {
                ...
                if (w == 0)
                {
                    debug.println("Further warnings of this type will "
                        ...
                        new Exception("Call trace");//.StackTrace;// printStackTrace();
            }
        }
...
private readonly static int I_KEY = 1;
private readonly static int I_PARAMSPEC = 2;
private readonly static int I_PARAMS = 3;
private readonly static int I_CERT = 4;
...
thisSpi.engineInit(opmode, key, random);
    ...
    internal class ProviderException
    {
        ...
        public ProviderException(string v)
        {
            this.v = v;
        }
    }
        ...
        lock(Lock)
{
    ...
    if (lastException is InvalidAlgorithmParameterException) {
     ...
     */
    public sealed Provider getProvider()
{
    chooseFirstProvider();
 ...
 */
public sealed string getAlgorithm()
{
 ...
 * not a block cipher
 */
public sealed int getBlockSize()
{
    chooseFirstProvider();
 ...
 */
public sealed int getOutputSize(int inputLen)
{

    if (!initialized && !(this is NullCipher)) {
 ...
 */
public sealed AlgorithmParameters getParameters()
{
    chooseFirstProvider();
 ...
 */
public sealed ExemptionMechanism getExemptionMechanism()
{
    chooseFirstProvider();
    return exmech;
}
...
private void checkCryptoPerm(CipherSpi checkSpi, Key key,
    ...
    private bool passCryptoPermCheck(CipherSpi checkSpi, Key key,
                                        AlgorithmParameterSpec Params)
//            throws InvalidKeyException
{
 ...
 */
public sealed void init(int opmode, Key key)// throws InvalidKeyException
{
 ...
 * by the underlying {@code CipherSpi}.
 */
public sealed void init(int opmode, Key key, SecureRandom random)
           // throws InvalidKeyException
{
    initialized = false;
    checkOpmode(opmode);
 ...
 * by the underlying {@code CipherSpi}.
 */
public sealed void init(int opmode, Key key, AlgorithmParameterSpec Params)
            //throws InvalidKeyException, InvalidAlgorithmParameterException
    {
 ...
 * by the underlying {@code CipherSpi}.
 */
public sealed void init(int opmode, Key key, AlgorithmParameters Params)
//            throws InvalidKeyException, InvalidAlgorithmParameterException
    {
        init(opmode, key, Params, JceSecurity.RANDOM);
    }
 ...
 * by the underlying {@code CipherSpi}.
 */
public sealed void init(int opmode, Certificate certificate)
//            throws InvalidKeyException
{
 ...
 */
public sealed byte[] update(byte[] input)
{
    checkCipherState();
 ...
 * (e.g., has not been initialized)
 */
public sealed byte[] update(byte[] input, int inputOffset, int inputLen)
{
    checkCipherState();
 ...
 * to hold the result
 */
public sealed int update(byte[] input, int inputOffset, int inputLen,
                        byte[] output)
 ...
 * @since 1.5
 */
public sealed int update(ByteBuffer input, ByteBuffer output)
//            throws ShortBufferException
{
    checkCipherState();
...
public sealed byte[] doFinal()
     ...
     * does not match the calculated value
     */
    public sealed int doFinal(byte[] output, int outputOffset)
     ...
     * does not match the calculated value
     */
    public sealed byte[] doFinal(byte[] input)
        //    throws IllegalBlockSizeException, BadPaddingException 
        {
        checkCipherState();
     ...
     * does not match the calculated value
     */
    public sealed byte[] doFinal(byte[] input, int inputOffset, int inputLen)
        //    throws IllegalBlockSizeException, BadPaddingException 
        {
        checkCipherState();
     ...
     * does not match the calculated value
     */
    public sealed int doFinal(byte[] input, int inputOffset, int inputLen,
                             byte[] output)
     ...
     * @since 1.5
     */
    public sealed int doFinal(ByteBuffer input, ByteBuffer output)
    //throws ShortBufferException, IllegalBlockSizeException,
    //BadPaddingException 
    {
        checkCipherState();
     ...
     */
    public sealed byte[] wrap(Key key)
 ...
 * {@code CipherSpi} is not supported.
 */
public sealed Key unwrap(byte[] wrappedKey,
...
//            throws InvalidParameterSpecException
{
    ...
    string alg = Params.getAlgorithm().ToUpper(Locale.ENGLISH);
...
string[] parts = tokenizeTransformation(transformation);
     ...
     * @since 1.5
     */
    public static sealed int getMaxAllowedKeyLength(string transformation)
//            throws NoSuchAlgorithmException
{
 ...
 * @since 1.5
 */
public static sealed AlgorithmParameterSpec getMaxAllowedParameterSpec(
 ...
 * @since 1.7
 */
public sealed void updateAAD(byte[] src)
{
    if (src == null)
    {
 ...
 * @since 1.7
 */
public sealed void updateAAD(byte[] src, int offset, int len)
{
    checkCipherState();
 ...
 * @since 1.7
 */
public sealed void updateAAD(ByteBuffer src)
{
    checkCipherState();
